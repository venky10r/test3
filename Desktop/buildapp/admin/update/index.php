    ...
    <div id="app">		
        ...
        <?php include('../include/header.php');?>
                                        ...
                                        <form name="f" method="post" action="" enctype="multipart/form-data">
                                            <div class="form-group">
                                                ...
                                                <select name="loc" class="form-control">
                                                   <?php
                                                    $query="select * from tblsite as s, tblallocate as a where a.engg='{$_SESSION['id']}' and s.siteid=a.siteid order by s.name";
                                            ...
                                            <button type="submit" name="sub" class="btn btn-o btn-primary">Get Details</button>
                                        </form><br/><br/>
                                        <?php 
			...
			<thead>
				...
				<tbody>
				<?php
					$query="select * from tblbook where accepted='Y' order by username";
					$result=mysqli_query($conn, $query);
					$sno=1;
					            while ($row = mysqli_fetch_assoc($result)){
                                                        $query="SELECT * FROM tblsite as s,tblsitedetails as d,tblusers as u where s.siteid='{$_POST['loc']}' and u.email='{$row['username']}' and d.id='{$row['bookid']}' and s.siteid=d.siteid";
                                                        ...
                                                        if(mysqli_num_rows($result1)){
                                                            ...
                                                            ?>
                                                            <td><a href="../updatedetails/?id=<?php echo $row['id'];?>&bookid=<?php echo $row['bookid'];?>" class="btn btn-transparent btn-xs" tooltip-placement="top" title="Update Status for Site"><i class="fa fa-cc-discover"></i></a></td></tr>
                                                            <?php
                                                            $sno++;
                                                        }
					            }
