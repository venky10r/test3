    ...
    <div id="app">		
        ...
        <?php include('../include/header.php');?>
			...
			<thead>
				...
				<?php
					$query  = "SELECT * FROM tblsite,tblsitedetails where tblsite.siteid = tblsitedetails.siteid";
					...
					$sno = 1;
					            ...
					            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
					                ...
					                echo "<td>" . $row['total'] ."</td>";
                                                        echo "<td>" . $row['builtup'] ."</td>";
                                                        echo "<td>" . $row['carpet'] ."</td>";
                                                        echo "<td>" . $row['rate'] ."</td>";
					                ?>
                                                        <td><a href="../editsitedetails?id=<?php echo $row['id'];?>&siteid=<?php echo $row['siteid'];?>" class="btn btn-transparent btn-xs" tooltip-placement="top" tooltip="Edit"><i class="fa fa-pencil"></i></a>													
                                                        <a href="?id=<?php echo $row['id']?>" onClick="return confirm('Are you sure you want to delete?')" class="btn btn-transparent btn-xs tooltips" tooltip-placement="top" tooltip="Remove"><i class="fa fa-times fa fa-white"></i></a></td>
                                                        <?php
					                $sno++;
					            }
					?>									
...
<?php
if(isset($_GET['id'])){
        $query="delete from tblsitedetails where id='{$_GET['id']}'";
        $result=mysqli_query($conn,$query) or die(mysqli_error($conn));
               echo "<script> alert('Site details deleted'); 
                   window.location.href = '../viewsitedetails'; </script>";
    }
?>
    <!-- start: FOOTER -->
