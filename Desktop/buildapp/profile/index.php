						...
						<div class="row">
							<div class="col-lg-6">
								<div class="property_info">
									<div class="tag_price listing_price col-md-12" style="text-align: center">Profile</div><div>&nbsp;</div>                                                                      
									<form action="" method="post" class="contact_form text-center" id="contact_form">
							<div class="row">
								<div class="col-md-12">
									<input type="text" name="fname" value="<?php echo $row['fname']; ?>" class="contact_input" placeholder="First name" pattern="[A-Za-z]+" title="Please enter characters" required="required" />
								</div>
								<div class="col-md-12">
									<input type="text" name="lname" value="<?php echo $row['lname']; ?>" class="contact_input" placeholder="Last name" pattern="[A-Za-z]+" title="Please enter characters" required="required" />
								</div>
								<div class="col-md-12">
									<input type="text" name="mob" value="<?php echo $row['mobile']; ?>" class="contact_input" placeholder="Mobile" pattern="[6,7,8,9][0-9]{9}" maxlength="10" title="Please enter 10 digit mobile number" required="required" />
								</div>
                                                                <div class="col-md-12">
									<textarea name="addr" class="contact_textarea contact_input" placeholder="Address" required="required" ><?php echo $row['address']; ?></textarea>
								</div>
							</div>
                                                                <button type="submit" name="profile" class="contact_button">Update Profile</button>
                                                                        </form>
								</div>
							</div>
                                                    ...
                                                    <div class="col-lg-6">
								<div class="property_info">
									<div class="tag_price listing_price col-md-12" style="text-align: center">Change Password</div><div>&nbsp;</div>                                                                      
									<form name="add" onsubmit="javascript:return valid();" method="post" action="" class="contact_form text-center">
							<div class="row">
								<div class="col-md-12">
									<input type="password" name="old" class="contact_input" placeholder="Current Password" required="required" />
								</div>
								<div class="col-md-12">
									<input type="password" name="pass" class="contact_input" placeholder="New Password" required="required" />
								</div>
								<div class="col-md-12">
									<input type="password" name="repass" class="contact_input" placeholder="Confirmed Password" required="required" />
								</div>
							</div>
                                                                <button type="submit" name="chpass" class="contact_button">Change Password</button>
                                                                        </form>
								</div>
							</div>
						</div>		
...
<?php
    if(isset($_REQUEST['profile'])){
        $query="update tblusers set fname='{$_POST['fname']}', lname='{$_POST['lname']}', address='{$_POST['addr']}', mobile='{$_POST['mob']}' where email='{$_SESSION['user']}'";
        $result=mysqli_query($conn,$query);
        if(mysqli_affected_rows($conn)>0){
             echo "<script> iqwerty.toast.Toast('Profile updated'); 
                         window.setTimeout(function() { window.location = '';  }, 4000);</script>";
        }else{
             echo "<script> iqwerty.toast.Toast('Error in updating the profile.'); 
                         window.setTimeout(function() { window.location = '';  }, 4000);</script>";
        }
    }
    
    if(isset($_REQUEST['chpass'])){
        $query="select * from tblusers where email='{$_SESSION['user']}' and binary password='{$_POST['old']}'";
        $result=mysqli_query($conn,$query);
        if(mysqli_num_rows($result)>0){
            $query="update tblusers set password='{$_POST['pass']}' where email='{$_SESSION['user']}'";
            $result=mysqli_query($conn,$query);
            if(mysqli_affected_rows($conn)>0){
                 echo "<script> iqwerty.toast.Toast('password updated'); 
                             window.setTimeout(function() { window.location = '';  }, 4000);</script>";
            }else{
                 echo "<script> iqwerty.toast.Toast('Error in updating the password.'); 
                             window.setTimeout(function() { window.location = '';  }, 4000);</script>";
            }
        }else{
            echo "<script> iqwerty.toast.Toast('Old password does not match'); 
                             window.setTimeout(function() { window.location = '';  }, 4000);</script>";
        }
    }
?>
