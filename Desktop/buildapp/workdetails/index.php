						...
						<div class="row">
							<div class="col-lg">
								<div class="property_info">
                                                                    <div class="tag_price listing_price col-md-12" style="text-align: center"><?php echo $_GET['site']; ?></div><div>&nbsp;</div>                                                                      
 <?php
 if(isset($_GET['wsiteid'])){
     $query="select * from tblworkimages where siteid='{$_GET['wbookid']}' and bookid='{$_GET['wsiteid']}' and dtdate='{$_GET['dt']}'";
...
<div id="jssor_1" style="position:relative;margin:0 auto;top:0px;left:0px;width:980px;height:380px;overflow:hidden;visibility:hidden;">
    ...
    while($row=mysqli_fetch_assoc($result)){ 
        $img="../workimages/".$row['filename'];
        
        ?>            
            <div>
                <img data-u='image' src='../workimages/<?php echo $row['filename'] ?>' />
...
<?php }
    ...
    echo "<h3>No images available for the date {$_GET['dt']}</h3>";
}

}?>
                                             <br/><br/><table class="table table-bordered datatable" id="table-1">
			<thead>
				<tr>
					<th>Sl No</th>
					<th>Date</th>
				...
				<tbody>
                                    <?php
					$query="select * from tblsitework where siteid='{$_GET['siteid']}' and bookid='{$_GET['bookid']}'";
					$result=mysqli_query($conn, $query);
					$sno=1;
					            while ($row = mysqli_fetch_assoc($result)){
                                                            echo "<tr><td>" . $sno . "</td>";
                                                            echo "<td>" . $row['dtdate'] ."</td>";
                                                            echo "<td>" . $row['workdone'];"</td>";
                                                            echo "<td><a href='?wbookid={$row['siteid']}&wsiteid={$row['bookid']}&bookid={$row['bookid']}&siteid={$row['siteid']}&site={$_GET['site']}&dt={$row['dtdate']}'>View Images</a></td>";
                                                            $sno++;
                                                        }
					?>	
                                </tbody>
